...
export function AppFooter({
  ...
  return (
    <footer className="border-t border-[#D9E6FF] bg-white/78">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="max-w-lg">
            <p className="inline-flex rounded-xl bg-[#12326B] px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.24em] text-white">
              Tecpify
            </p>
            <h2 className="mt-4 text-lg font-semibold text-[#12326B]">
              {variant === "workspace"
                ? "Operacion y visibilidad ligera para negocios en crecimiento."
                : "Ordena tus pedidos, comparte tu link y opera con mas claridad."}
            </h2>
            <p className="mt-2 text-sm leading-6 text-[#294B8F]">
          ...
          <nav aria-label="Enlaces del footer" className="flex flex-wrap gap-2">
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-full border border-[#D9E6FF] bg-white px-4 py-2 text-sm font-medium text-[#294B8F] transition hover:border-[#BFD3FF] hover:bg-[#F8FBFF] hover:text-[#12326B]"
              >
                {link.label}
              </Link>
            ))}
        ...
        <div className="flex flex-col gap-2 border-t border-[#D9E6FF] pt-4 text-sm text-[#294B8F] sm:flex-row sm:items-center sm:justify-between">
