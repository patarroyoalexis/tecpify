...
interface OrderByIdRouteDependencies {
  requireOrderApiContext: typeof requireOrderApiContext;
  updateOrderInDatabase: typeof updateOrderInDatabase;
}

export function createOrderByIdRouteHandlers(
  dependencies: OrderByIdRouteDependencies = {
  ...
  return {
    async PATCH(
      ...
      const invalidFields = fieldsUpdated.filter(
        (field) => !UPDATE_ORDER_ALLOWED_FIELDS.has(field),
      );
...
const orderByIdRouteHandlers = createOrderByIdRouteHandlers();

export const PATCH = orderByIdRouteHandlers.PATCH;
